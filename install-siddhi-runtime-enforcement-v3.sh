...
INSTALLER_REVISION="2026-07-07-v3"
...
printf '[siddhi-runtime-install] Installer revision: %s\n' "$INSTALLER_REVISION"
...
# Locate the generated Python list using regular expressions, then insert
# immediately before the list's closing bracket. This tolerates tabs, spaces,
# blank lines and repository formatting changes.
import re
...
if f'"name": "{service_name}"' not in text:
    ...
    if not list_match or not operation_match or operation_match.start() <= list_match.end():
        raise SystemExit(
            f'Cannot patch {p}: generated services list boundaries were not found'
        )

    list_region = text[list_match.end():operation_match.start()]
    close_relative = list_region.rfind(']')
    if close_relative < 0:
...
# Extend the shell verification array using regex boundaries rather than an
...
expected_region = text[expected_match.end():loop_start]
if service_name not in expected_region:
    close_relative = expected_region.rfind(')')
    if close_relative < 0:
    ...
    if before and not before.endswith('\n'):
        before += '\n'
    text = before + f'  {service_name}\n' + text[insert_at:]

text = text.replace(
    'All five MI services are registered.',
    'All six MI services are registered.',
)
