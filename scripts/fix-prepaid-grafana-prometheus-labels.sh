...
fail() {
  printf '[fix-prepaid-grafana-labels][FAIL] %s\n' "$*" >&2
  exit 1
}

pass() {
  printf '[fix-prepaid-grafana-labels][PASS] %s\n' "$*"
}
...
BACKUP_DIR=".prepaid-reconciliation-backups/grafana-labels-$STAMP"
...
pass "Removed hardcoded Prometheus job and instance labels."
pass "The verifier now reports progress while waiting."
