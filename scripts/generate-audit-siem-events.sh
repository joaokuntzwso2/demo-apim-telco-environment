...
if (( sim_throttled < 1 )); then
  ...
  # audit-seed-nonfatal-429-v2
  if [[ "${AUDIT_SEED_STRICT:-false}" == true ]]; then
    exit 1
  fi
  echo "[audit-events][WARN] Native HTTP 429 was not observed; continuing the automatic demo startup." >&2
fi
