...
// developer-experience-product-state-reconciliation-v2
async function listApiProducts(token) {
  ...
  /*
   * API Product creation and Publisher collection indexing may not become
   * visible at exactly the same moment. The Product bootstrap state already
   * contains the authoritative UUID returned by APIM, so use it to supplement
  ...
  for (let attempt = 1; attempt <= 10; attempt += 1) {
    try {
      ...
      log(
        `API Product collection lookup failed on attempt `
        + `${attempt}: ${error.message}`
      );
    }

    try {
      if (fs.existsSync(productStateFile)) {
        ...
        for (const entry of state.products || []) {
          ...
          try {
            ...
            log(
              `resolved API Product from bootstrap state: `
              + `${product.name}:${product.version} `
              + `(${product.id})`
            );
          } catch (error) {
            log(
              `could not read state-backed API Product `
      ...
      log(
        `could not read API Product bootstrap state: `
        + error.message
      );
    }
    ...
    const missingNames =
      Array.from(TARGET_PRODUCT_NAMES)
        .filter(
    ...
    if (attempt < 10) {
      log(
        `waiting for API Product visibility; missing: `
        + missingNames.join(', ')
      );

      await sleep(
        Math.min(attempt * 500, 3000)
      );
    }
  }
...
`
  ...
  , {
      name: '10 - Live Gateway Analytics',
      summary: 'Near-real-time APIM Gateway analytics, commercial dimensions and privacy controls.',
      type: 'HOWTO',
      content: `${DOC_MARKER}
# Live Gateway Analytics

Every invocation through WSO2 API Manager is published asynchronously to Moesif by the native APIM 4.7 analytics reporter. Successful Gateway processing and faulty/rejected Gateway events are both captured.

## Captured dimensions

- Native APIM fields: API, version, resource, HTTP method, application, Gateway type/region, proxy and backend response status, request/mediation/backend latency and correlation identifier.
- Telco dimensions under the event properties: partner, API Product, country, named Gateway, subscription identifier/policy, commercial plan, billable units and transaction outcome (\`SUCCESS\`, \`FAILED\` or \`REJECTED\`).
- A backend or business failure remains distinguishable from a Gateway rejection. When MI returns an allowed degraded/partial response, the HTTP result remains truthful while the telco outcome can identify the failed downstream business stage.

## Consumer headers for the sandbox

Use the application subscription and OAuth token generated by the Developer Portal. The following optional, non-secret headers improve the demo segmentation:

- \`X-Correlation-ID\`
- \`X-Partner-ID\` or \`source-id\`
- \`X-Country-Code\` or \`organization-id\`
- \`X-Subscription-ID\`
- \`X-Subscription-Policy\`
- \`X-Commercial-Plan\`
- \`X-Billable-Units\`

These headers do not replace APIM authentication, authorization, scope, subscription or rate-limit enforcement. Server-side APIM context is preferred whenever available.

## Privacy and consent

The integration is configured with \`send_headers = false\`. Request/response bodies, bearer tokens, cookies and API keys are not exported. Only the APIM native analytics schema and an explicit allowlist of business metadata are sent. Use synthetic sandbox identities and follow the consent/legal-basis guidance in this API's Consent and Privacy document.

## Errors, SLA and retries

- \`401\`, \`403\`, policy/schema rejection and \`429\` appear as \`REJECTED\`.
- Backend/connectivity and server failures appear as \`FAILED\`.
- Completed invocations appear as \`SUCCESS\`.
- Analytics publishing is asynchronous and outside the business request path. The demonstration objective is visibility within seconds, but external SaaS availability and ingestion delay are not part of the API's transactional SLA.
- Consumers must continue to follow the API Error Catalogue and bounded retry rules; an analytics event is evidence, not permission to retry a non-idempotent operation.

## Postman and SDKs

Import \`artifacts/postman/telco-live-moesif-analytics.postman_collection.json\` for successful, failed and rejected examples. Generated SDK users should set the same non-secret metadata headers through the client's request-header mechanism and always preserve \`X-Correlation-ID\`.
`
    }];
}
...
`
  ...
  , {
      name: '06 - Live Gateway Analytics',
      summary: 'Product-level usage, partner segmentation, plan attribution and billable-unit guidance.',
      type: 'HOWTO',
      content: `${DOC_MARKER}
# Live Gateway Analytics for this API Product

Member-API invocations are sent by the WSO2 API Manager 4.7 Gateway to Moesif in near real time. The event retains the native API and operation while adding this API Product, partner, country/Gateway, subscription policy, commercial plan, billable units, correlation identifier and \`SUCCESS\` / \`FAILED\` / \`REJECTED\` classification.

Use the member API's Authentication, Consent, Error Catalogue, Commercial Plan, SLA, Sandbox and Live Gateway Analytics documents. The API Product subscription and APIM runtime policy remain authoritative; analytics metadata does not grant access or change billing.

No payload, bearer token, cookie or secret header is sent. Postman examples are available in \`artifacts/postman/telco-live-moesif-analytics.postman_collection.json\`, and generated SDKs can pass the documented non-secret metadata headers.
`
    }];
}
