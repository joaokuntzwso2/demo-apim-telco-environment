...
function createRevisionAndDeploy({ apimUrl, token, apiId, log }) {
  ...
  const revision = runCurlJson([
    ...
    '-d', JSON.stringify({ description: 'Automated streaming API revision for local demo.' }),
...
function publishApi({ apimUrl, token, apiId, log }) {
  try {
    ...
    if (String(e.message || '').includes('already') || String(e.message || '').includes('PUBLISHED')) {
      log(`Streaming import: API already published.`);
      return;
    }
    throw e;
  }
...
function importStreamingApi({
  ...
  const created = runCurlJson([
    '-X', 'POST',
    '-H', `Authorization: Bearer ${token}`,
    '-F', `file=@${asyncapiPath};type=application/yaml`,
  ...
  log(`Streaming import: created unpublished working copy ${name}:${version} as ${api.type || normalizedType} (${api.id})`);

  return api;
}

module.exports = { importStreamingApi };
