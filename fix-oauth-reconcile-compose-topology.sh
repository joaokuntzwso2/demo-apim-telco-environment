...
log() {
  printf '[oauth-compose-topology-fix] %s\n' "$*"
}

fail() {
  printf '[oauth-compose-topology-fix][FAIL] %s\n' "$*" >&2
  exit 1
}
...
backup_dir=".restart-backups/oauth-compose-topology-${timestamp}"
mkdir -p "$backup_dir"

if [[ -f "$TARGET" ]]; then
  cp "$TARGET" "$backup_dir/reconcile-oauth-control-plane.sh"
fi

log "Backup written under $backup_dir"

cat > "$TARGET" <<'RECONCILE'
#!/usr/bin/env bash
set -Eeuo pipefail
...
log "Validating replacement script"
bash -n "$TARGET"

grep -q \
  'com.docker.compose.project.config_files' \
  "$TARGET" ||
  fail "Running Compose topology discovery was not installed."

grep -q \
  'Resolved apim-bootstrapper in Compose topology' \
  "$TARGET" ||
  fail "Bootstrapper topology validation was not installed."

cat <<EOF

[oauth-compose-topology-fix] Fix installed successfully.

The reconciliation process now uses the project metadata from the running
WSO2 APIM container instead of rebuilding a guessed Compose topology.
