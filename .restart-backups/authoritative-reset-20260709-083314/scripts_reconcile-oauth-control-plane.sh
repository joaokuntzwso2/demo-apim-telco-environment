...
log "Reconciling the two-second OAuth client lifetime through the APIM key-mapping API."
bash scripts/reconcile-short-lived-oauth-client.sh

log "Generating the MI persona registry from authoritative SCIM user IDs."
...
"${OAUTH_COMPOSE[@]}" run \
  ...
  -lc '
    set -Eeuo pipefail
    echo "[oauth-reconcile] Global Developer Experience is owned by the base bootstrap and is skipped here."
  '
