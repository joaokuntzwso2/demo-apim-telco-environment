...
async function listAndFindApi(token) {
  const response = await request(
    `${APIM_URL}/api/am/publisher/v4/apis?limit=1000&query=${encodeURIComponent(`name:${API_NAME}`)}`,
    { headers: bearer(token) }
  );
  const list = response.data?.list || response.data?.data || response.data || [];
  for (const summary of list) {
    if (summary.name !== API_NAME || String(summary.version) !== API_VERSION) continue;
    const full = await request(`${APIM_URL}/api/am/publisher/v4/apis/${summary.id}`, {
      headers: bearer(token)
    });
...
async function upsertApi(token) {
  ...
  let changed = false;
  const currentMarker = (api?.additionalProperties || []).find(item => item.name === 'SecurityControlModel')?.value;
  const currentScopeKeys = new Set(
    (api?.scopes || [])
      .map(item => item?.scope?.name || item?.name)
      .filter(Boolean)
  );
  const completeScopes = SCOPES.every(scope => currentScopeKeys.has(scope.key));
  ...
  if (!api) {
  ...
  } else if (currentMarker !== MARKER || !completeScopes) {
    ...
    log(`API ${api.id} already carries ${MARKER}`);
  }
  ...
  await request(
    `${APIM_URL}/api/am/publisher/v4/apis/${api.id}/swagger`,
    { method: 'PUT', headers: bearer(token), body: form },
    [200]
  );
  log('Updated managed OpenAPI definition');

  if (changed) {
    const deploymentsResult = await request(`${APIM_URL}/api/am/publisher/v4/apis/${api.id}/deployments`, {
      headers: bearer(token)
    });
    ...
    if (!Array.isArray(deployments) || deployments.length === 0) {
      const revision = await request(`${APIM_URL}/api/am/publisher/v4/apis/${api.id}/revisions`, {
        method: 'POST',
        headers: bearer(token, { 'content-type': 'application/json' }),
        body: JSON.stringify({ description: 'OAuth scopes, roles, consent and risk-based authorization' })
      });
      await request(
        `${APIM_URL}/api/am/publisher/v4/apis/${api.id}/deploy-revision?revisionId=${encodeURIComponent(revision.data.id)}`,
        {
          method: 'POST',
          headers: bearer(token, { 'content-type': 'application/json' }),
      ...
      log(`API already has ${deployments.length} deployment(s); preserving current deployed revision`);
    }
  }

  const refreshed = await request(`${APIM_URL}/api/am/publisher/v4/apis/${api.id}`, {
    headers: bearer(token)
  });
  if (String(refreshed.data?.lifeCycleStatus || '').toUpperCase() !== 'PUBLISHED') {
    await request(
      `${APIM_URL}/api/am/publisher/v4/apis/change-lifecycle?action=Publish&apiId=${encodeURIComponent(api.id)}`,
